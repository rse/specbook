...
-   `SchemaDiagram.config?: SchemaGradiaConfig`:
    arbitrary Gradia rendering options (e.g. `grid-columns-max: 2`),
...
rendering options (e.g. `grid-columns-max: 2`) through to the diagram.
...
`grid-gap-vertical: 12`, and `grid-columns-min: 4` (the square
derivation of the column count otherwise leaves a third of the document
width unused).
...
SVG files are inlined as-is; PNG/JPEG/WebP files are embedded as base64
