...
/*  the supported compression levels of the emitted YAML schema
    ...
    without comments (1), additionally without its "refs", "diagram", and
    "format" fields (2), and additionally without its "desc" fields of
    objects and properties (3)  */
...
const compressYaml = (yaml: string, level: CompressLevel): string => {
    ...
    const drop = [ [ "refs", "diagram", "format" ], [ "desc" ] ].slice(0, level - 1).flat()
